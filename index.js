...
app.get('/verify', async (req, res) => {
    ...
    const tokenDoc = await db.collection('verify_tokens').findOne({ token, is_used: false });
    ...
    <body>
        ...
        <script>
            async function processVerify() {
                ...
                try {
                    const res = await fetch('/api/process-token?token=${token}');
...
app.get('/api/process-token', async (req, res) => {
    ...
    // Single-use guarantee: Atomically find & delete token
    const tokenDoc = await db.collection('verify_tokens').findOneAndDelete({ token: token, is_used: false });

    if (!tokenDoc || !tokenDoc.value) {
        return res.json({ success: false, message: "Token already used or expired!" });
    }

    // Dynamic Database Settings Read
    ...
    try {
        ...
        const targetTelegramUrl = `https://t.me/${botUsername}?start=verify_${token}`;
